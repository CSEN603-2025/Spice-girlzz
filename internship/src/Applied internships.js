...
function AppliedInternships() {
  ...
  useEffect(() => {
    if (appliedInternships.length === 0 && !sessionStorage.getItem('appliedInternships')) {
  ...
  const pendingInternships = appliedInternships
    .filter((internship) => ['pending', 'finalized', 'accepted', 'rejected'].includes(internship.status))
    .filter((internship) => {
      const matchesSearch =
      ...
      const matchesDate = filterDate
        ? filterDate === 'recent'
          ? new Date(internship.applicationDate) >= new Date(appliedInternships.map(i => i.applicationDate).sort().slice(-1)[0])
          : new Date(internship.applicationDate) <= new Date(appliedInternships.map(i => i.applicationDate).sort()[0])
        : true;
      return matchesSearch && matchesStatus && matchesDate;
    });

  const currentAndCompletedInternships = appliedInternships
    .filter((internship) => ['current', 'completed'].includes(internship.status))
    .filter((internship) => {
      const matchesSearch =
        internship.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        internship.company.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <div style={styles.container}>
      ...
      <div style={styles.layout}>
        ...
        <div style={styles.sidebar}>
          <SideBar setActivePage={(page) => navigate(`/student${page === 'home' ? '' : '/' + page}`)} />
        ...
        <main style={{ flex: 1, padding: '1.5rem', overflowY: 'auto' }}>
          ...
          <div style={{ background: '#fff', padding: '1.5rem', borderRadius: '0.375rem', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)', marginBottom: '2rem' }}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setAppliedInternships((prev) =>
                  prev.map((internship) =>
                    internship.id === selectedInternship.id ? { ...internship, status: newStatus } : internship
                  )
                );
